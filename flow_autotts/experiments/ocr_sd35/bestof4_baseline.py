"""Run a sharded best-of-4 ODE baseline and build a compact summary."""
...
def _bestof4_behavior_summary(per_candidate_nfe: int, total_nfe: int) -> str:
    return (
        "best-of-4 deterministic ODE "
        f"(spawn=4.00, forward={float(per_candidate_nfe):.2f}x4, "
...
def _bestof4_answer(
    *,
    ...
    if total_nfe % candidates != 0:
        raise ValueError(f"total_nfe={total_nfe} must be divisible by candidates={candidates}")

    per_candidate_nfe = total_nfe // candidates
    ...
    for candidate_index in range(candidates):
        reward, answer_nfe = _deterministic_ode_answer(
            resources=resources,
            prompt=prompt,
            seed=int(seed + candidate_index),
            total_nfe=per_candidate_nfe,
            resolution=resolution,
            guidance_scale=guidance_scale,
            noise_level=noise_level,
            sde_type=sde_type,
        ...
        if reward is not None and (best_reward is None or float(reward) > float(best_reward)):
            best_reward = float(reward)
            best_index = candidate_index

    return best_reward, total_nfe, best_index, candidate_results
...
def _evaluate_worker(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for local_rank, sample in enumerate(samples):
        ...
        reward, answer_nfe, best_candidate_index, candidate_results = _bestof4_answer(
            resources=resources,
        ...
        episodes.append(
            {
                ...
                "answer_nfe": int(answer_nfe),
                "total_nfe": total_nfe,
                "per_candidate_nfe": per_candidate_nfe,
    ...
    action_statistics = {
        "answer": 1.0,
        "forward": float(per_candidate_nfe),
        "mean_nfe": float(total_nfe),
        "spawn": float(candidates),
    }
    raw_result = {
        "beta": beta,
        ...
        "behavior_summary": _bestof4_behavior_summary(per_candidate_nfe, total_nfe),
    }
    compact_row = {
        "beta": beta,
    ...
    round_result = {
        "round_id": 0,
        "controller_name": "BestOf4OdeBaseline",
        "controller_key": "bestof4_ode",
...
def _run_main(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for beta, target_nfe in zip(betas, target_nfes, strict=True):
        ...
        if int(target_nfe) % candidates != 0:
            raise ValueError(f"target_nfe={target_nfe} must be divisible by candidates={candidates}")
    ...
    for beta, total_nfe in zip(betas, target_nfes, strict=True):
        ...
        for shard_index, device in enumerate(devices):
            ...
            cmd = [
                sys.executable,
                "-m",
                "flow_autotts.experiments.ocr_sd35.bestof4_baseline",
                "--worker",
                "--repo-root",
                str(repo_root),
                "--dataset",
        ...
        run_manifest["runs"].append(
            {
                ...
                "per_candidate_nfe": int(total_nfe // candidates),
                "candidates": candidates,
