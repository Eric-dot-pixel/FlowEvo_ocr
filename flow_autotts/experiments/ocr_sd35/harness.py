"""Run controller rounds on random OCR train prompts with SD3.5."""
...
from flow_autotts.experiments.ocr_sd35.dataset import PromptSample, sample_prompt_file
...
def run_harness(
    ...
    ocr_model = Path(ocr_model_path) if ocr_model_path is not None else None
    ...
    history: dict[str, Any] = {
        ...
        "ocr_model_path": str(ocr_model) if ocr_model is not None else "",
        "device": runtime_device,
...
def _default_ocr_path() -> Path | str:
    return REPO_ROOT / "flow_grpo"


def _default_output_path() -> Path:
    return REPO_ROOT / "logs" / "flow_autotts" / "ocr_sd35" / "history.json"
